...
// <!--
    ...
    function toeUpdateTemplate(newValue) {
        jQuery(this).sendForm({
            data: {
                reqType: 'ajax', action: 'putOption', page: 'optionsGmp', code: 'default_theme', value: newValue
            },
            msgElID: 'toeTplMsg',
            onSuccess: function(res) {
                if(!res.error) {
                    jQuery('#toeTplMsg').html('<?php langGmp::_e('Template Updated. Reload page to see results.')?>');
...
?>
                <td width="<?php echo ceil(100/$tplPerRow)?>%">
                    <div class="toeTplOption">
                        ...
                        <a href="<?php echo $code?>" class="toeTplActivateLink"><?php langGmp::_e('Activate')?></a>
                        <a href="<?php echo $code?>" class="toeTplDeactivateLink"><?php langGmp::_e('Deactivate')?></a><br /><br />
